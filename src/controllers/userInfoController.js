const { fetchUserInfo } = require('../services/userInfoService');
...
const userInfoController = async (req, res) => {
  ...
  try {
    const profileData = await fetchUserInfo(username, password);
    res.status(200).json({ success: true, data: profileData });
  } catch (error) {
    console.error('Lỗi khi cào dữ liệu hồ sơ sinh viên:', error);
    res.status(500).json({
      success: false,
      message: 'Không thể cào dữ liệu hồ sơ sinh viên',
      error: error.message,
    });
  }
};
